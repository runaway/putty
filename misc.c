...
//AfxMessageBox("Misc bufchain_add");
    if (len == 0) return;
...
#else
	unsigned int nCopyLen = 3;
	char szTemp[3];
	int iTemp = *(unsigned char *)data;

	if (ch->tail && ch->tail->bufend < ch->tail->bufmax) 
	{
	    int copylen = min(len, ch->tail->bufmax - ch->tail->bufend);
		//memset(buf + copylen - 1, 32, 1);
		//memcpy(buf, "jc ", 3);
		itoa(iTemp, szTemp, 16);

		if (0xF >= iTemp)
		{
			memset(szTemp + 1, 0x31, 1);
		}
		
		memset(&szTemp[2], 0x20, 1);
		memcpy(ch->tail->bufend, szTemp, nCopyLen);
	    //memcpy(ch->tail->bufend, buf, copylen);
		//memset(buf, 32, 1);
		//memcpy(ch->tail->bufend, buf, copylen);
